...
if iter < 2.5
  ...
  lowmin = max(0,xval - 10*(xmax-xmin));
  lowmax = xval - 0.01*(xmax-xmin);
  uppmin = xval + 0.01*(xmax-xmin);
  uppmax = min(1,xval + 10*(xmax-xmin));
  %lowmin = xval - 10*(xmax-xmin);
  %lowmax = xval - 0.01*(xmax-xmin);
  %uppmin = xval + 0.01*(xmax-xmin);
  %uppmax = xval + 10*(xmax-xmin);

  % lowmin = max(0, xval - 2*(xmax - xmin));
  % lowmax = xval - 0.005*(xmax - xmin);
  % uppmin = xval + 0.005*(xmax - xmin);
  % uppmax = min(1, xval + 2*(xmax - xmin));
